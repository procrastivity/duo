...
// PermissionRequest event is intentionally absent because the exact-version
// projection evidence does not establish that event.
...
func MaterializeProjection(workspace, installationID string) error {
	...
	stamp := projectionStamp{
		...
		Target: projectionTarget{
			Harness:            "devin",
			TestedVersionRange: TestedVersionRange(),
		},
...
# installed for the verified exact-version projection.
set -eu
umask 077
...
func verifyProjectionFiles(workspace string, stamp projectionStamp) ProjectionStatus {
	...
	if stamp.Target.TestedVersionRange != TestedVersionRange() {
		return ProjectionStale
	}
...
func InspectProjection(workspace string, active []ProjectionActiveLaunch) ProjectionInspection {
	...
	if stamp.Target.TestedVersionRange != TestedVersionRange() {
		...
		inspection.Detail = fmt.Sprintf("the Devin projection targets version policy %q; this build uses %q", stamp.Target.TestedVersionRange, TestedVersionRange())
		return inspection
	}
